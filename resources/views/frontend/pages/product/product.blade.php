        ...
        <div class="col">
            <div class="single-widget range">
                <h3 class="title text-center"
                    style="padding-top:80px; color: #fa8888; text-decoration: none; text-transform: uppercase;">
                    Shop by Price
                </h3>
                            ...
                            <div class="product_filter mt-3">
                                <div class="label-input">
                                    ...
                                    <input type="text" id="amount" readonly
                                        style="border: 0; font-weight: bold; color: #fa8888;">
                                    <input type="hidden" name="price_range" id="price_range"
                                        value="{{ request()->get('price_range', '') }}">
                                </div>
                                <button type="submit" class="filter_button btn btn-primary mt-2"
                                    style="background-color: #fa8888; border-color: #fa8888;">
                                    Filter
                                </button>
                            </div>
                        </form>
                ...
                <a href="{{ route('productCart', $product->id) }}" style="text-decoration: none; color: inherit;">
                    <div class="productCard">
                        <img src="{{ $product->hasMedia('') ? $product->getMedia('')[0]->getFullUrl() : 'https://srv-np.techarttrekkies.com/placeholder/300x300/ffdfdf/EE00FF&text=' . str_replace(' ', '+', $product->name) }}"
                            ...
                            style="width: 400px; padding:10px; height: 200px; object-fit: cover;" alt="{{ $product->name }}">
                        <h5 class="productName">{{ $product->name }}</h5>
                        <p class="productDescription">
                            {{ $product->description == null ? '' : Str::limit($product->description, 60, '...') }}</p>
                    </div>
                </a>
